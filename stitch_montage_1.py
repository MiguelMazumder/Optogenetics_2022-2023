...
#CUR_DIR = os.getcwd()
CUR_DIR = r'W:\Senior_Design\Summer_Miguel\july1stweek\230707_122410_plate2_quant_pattern\230707_122410_Plate 1'
os.chdir(CUR_DIR)
...
def stitch_images(curr_images):
    ...
    is_integers = np.issubdtype(dim_tile_array.dtype, np.integer)
    if is_integers:
        print("The array contains only integers.")
    else:
        print("I hate this")
...
def switch_and_write(input_image,img_type,well_location,stitch_folder,original_folder):
    ...
    tf.imwrite(stitch_folder+r'\\'+well_location+'_'+img_type+'.tif', input_image)
