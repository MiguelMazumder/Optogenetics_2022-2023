...
% FIX a function here to add patterning to each illumination period (i.e
% pattern number instead of binary
framerate=1/60;
...
function [wells,wellDiam,xval,yval] = base_img(pxX,pxY,rows,cols)
    switch rows
        case 4
            leftBorder = 370;%double plate is 420 FIX SPACING FOR PLATE TYPES
            topBorder = 0;
            ...
            Xedgecorr = 195; %empirically slide the image left/right %double plate at 0 FIX SPACING FOR PLATE TYPES
            Yedgecorr = -30; %epirically slide the image up/down %started at -130 FIX SPACING FOR PLATE TYPES
            wellDiam = 124; %px 125 originally must be divisible by 2
        ...
        case 8
            ...
            %Spacing between wells is 2.6mm --> 31px
            Xedgecorr = 95; %empirically slide the image left/right %double plate at 0 FIX SPACING FOR PLATE TYPES
            Yedgecorr = 0; %epirically slide the image up/down %started at -130 FIX SPACING FOR PLATE TYPES
    ...
    for i = 1:rows%alter if we move to 24 well plate
        ...
        for j = 1:cols%alter if we move to 24 well plate
            ...
            %yCenter = yCenter + wellDiam + wellSpace;
            xCenter = xCenter + wellDiam + wellSpace;
        end
    end
...
function [] = write_video(time_store,framerate,well,wellDiam,unique_frames,xval,yval)%going to need to add black image before and after just as precaution
    ...
    writerObj = VideoWriter('test.avi');%,'Uncompressed AVI');can affect synchronization of the frames if not enough storage space
    ...
    % convert the image to a frame
        ...
        rgb_img=cat(3, red_frame, green_frame, blue_frame);
...
function [start_end_times, well_locs, row_num, col_num] = userinput_temp()
    ...
    plate_choice = questdlg('Select a plate type:', 'User Input', '24 well', '48 well', '96 well','24 well');
    plate_num = sscanf(plate_choice, '%d');% Convert plate type to double
    if plate_num == 24
        row_num=4;col_num = 6;wellDiam=124;assignin('base', 'wellDiam', wellDiam);%Testing
    elseif plate_num == 48
    ...
    if contains(device_choice, 'Projector')
        ...
        while ~isempty(char(answer))
            ...
            images=findimgs(200,wellDiam);%set up default images in folder
...
uiwait;%Testing
    ...
    function selection(~,~)%src,event
        ...
        %disp(['Selection: ' val]);
        welldiam = evalin('base', 'wellDiam');
        image_use=findimgs(400,welldiam);%400 is just the default for now
...
function [image] = findimgs(crop_,wellDiam)
    ...
    for i = 1:numel(allFiles)
        ...
        if imageSize(1)<crop_ || imageSize(2) < crop_ || isnan(crop_)
            crop_=imageSize(1);
        end
        % ensure crop will affect image correctly
        cropX = min(floor((size(img, 1) - crop_) / 2),floor((size(img, 2) - crop_) / 2));
        %cropY = floor((size(img, 1) - target_height) / 2);
        croppedImage = imcrop(img, [cropX, cropX, crop_-1, crop_-1]);
        ...
        grey_img=rgb2gray(resizedImage);%greyscale the image
